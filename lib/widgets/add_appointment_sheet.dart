...
class _AddAppointmentSheetState extends State<AddAppointmentSheet> {
        ...
        return Theme(
          data: Theme.of(context).copyWith(
            colorScheme: Theme.of(
              context,
            ).colorScheme.copyWith(primary: _blue600),
          ),
          child: child!,
        );
      },
    ...
    if (picked != null) {
      setState(() {
        ...
        if (_selectedSlot != null &&
            _isSlotStartInPast(_date, _selectedSlot!)) {
          _selectedSlot = null;
        }
      });
      await _loadBookedForDoctorDay();
  ...
  Future<void> _save() async {
    ...
    if (_selectedPatientPhone == null ||
        _selectedPatientPhone!.trim().isEmpty) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Could not load patient phone'),
          behavior: SnackBarBehavior.floating,
        ),
      );
      return;
                      ...
                      if (widget.prefilledDoctorId != null) ...[
                        ...
                        DropdownButtonFormField<String>(
                          ...
                          onChanged: (v) async {
                            ...
                            setState(() {
                              _selectedDoctorId = v;
                              _selectedDoctorName = selected.isNotEmpty
                                  ? selected.first.name
                                  : null;
                            });
                            await _loadBookedForDoctorDay();
                      ...
                      Wrap(
                        ...
                        children: _durationOptions.map((d) {
                          ...
                          return ChoiceChip(
                            ...
                            side: BorderSide(
                              color: isSelected ? _blue600 : _slate200,
                            ),
                            onSelected: (_) =>
